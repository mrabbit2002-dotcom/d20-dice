...
$work = Join-Path $env:RUNNER_TEMP 'd20-dice-build'
...
$env:CI = '1'
Push-Location $tpl
try { & pwsh .\.github\scripts\Build-Windows.ps1 -Target x64 -Configuration Release } finally { Pop-Location }
$dll = Get-ChildItem "$tpl\release" -Recurse -Filter 'd20-dice.dll' | Select-Object -First 1
if (-not $dll) { throw 'd20-dice.dll was not produced.' }
$out = Join-Path $root 'dist'; $payload = Join-Path $out 'payload'
New-Item "$payload\obs-plugins\64bit" -ItemType Directory -Force | Out-Null
Copy-Item $dll.FullName "$payload\obs-plugins\64bit\d20-dice.dll" -Force
